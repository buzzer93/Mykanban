...
- en mobile (`< 1024px` ou `pointer: coarse` jusqu'a `1279px`): une seule colonne visible a la fois, navigation horizontale native via `overflow-x` + `scroll-snap`, avec validation du changement de colonne par seuil de swipe strict (distance minimale du doigt, independante de l'inertie de scroll) pour eviter les bascules accidentelles, et recentrage immediat en fin de geste (sans lissage) pour eviter l'effet "swipe en deux temps"; boutons `←` / `→` sur chaque carte pour changer de colonne sans drag tactile

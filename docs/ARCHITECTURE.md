...
- en mobile (`< 1024px` ou `pointer: coarse` jusqu'a `1279px`): une seule colonne visible a la fois, navigation par geste tactile horizontal interprete dans `board_mobile_controller` puis rendu deterministe via `transform: translate3d(...)` de la track (pas de scroll-snap natif), avec seuil de validation renforce et cooldown court post-swipe pour limiter les bascules accidentelles et garantir un seul changement de colonne maximum par geste; boutons `←` / `→` sur chaque carte pour changer de colonne sans drag tactile

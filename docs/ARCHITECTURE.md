...
- en mobile (`< 1024px` ou `pointer: coarse` jusqu'a `1279px`): une seule colonne visible a la fois, navigation horizontale native via `overflow-x` + `scroll-snap` avec `scroll-snap-stop: always` pour eviter les sauts de plusieurs colonnes, et alignement initial sur la premiere colonne non vide au chargement; boutons `←` / `→` sur chaque carte pour changer de colonne sans drag tactile

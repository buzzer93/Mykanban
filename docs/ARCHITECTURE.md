...
- en mobile (`< 1024px` ou `pointer: coarse` jusqu'a `1279px`): une seule colonne visible a la fois, montee dans une track `display: flex` translatee horizontalement via `transform: translateX(-N * 100%)`, navigation par swipe horizontal sur toute la zone board (meme quand la colonne active est vide), boutons `‹` / `›` autour du compteur "1/N", et boutons `←` / `→` sur chaque carte pour changer de colonne sans drag
